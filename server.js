...
async function findListByIdOrToken(identifier) {
  ...
  for (const key in store.lists) {
    if (
      store.lists[key].share_token === identifier ||
      store.lists[key].id === identifier
    ) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (method === "GET" && sseMatch) {
    ...
    res._clientId = crypto.randomUUID
      ? crypto.randomUUID()
    ...
    if (newTitle) {
      ...
      if (conflict) {
        return sendJSON(res, 400, {
          error: `A list named "${newTitle}" already exists.`,
        })
      }
  ...
  if (method === "DELETE" && addItemMatch) {
    ...
    const mode =
      parsedUrl.query.mode ||
      (parsedUrl.query.completed === "true" ? "completed" : "all")
    let removedItems = []
  ...
  const updateItemMatch = pathname.match(
    /^\/api\/lists\/([^/]+)\/items\/([^/]+)$/,
  )
  ...
  if (
    ...
    (!fs.existsSync(filePath) && !path.extname(pathname))
  ) {
...
server.listen(PORT, () => {
  ...
  console.log(
    `  Cloud DB: ${isCloudConfigured ? "Connected to Supabase (" + SUPABASE_URL + ")" : "Local disk fallback"}`,
  )
